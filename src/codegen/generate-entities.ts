 ...
 * `subgraph-snapshot` module's `Entities` interface with one entry per
...
export interface GenerateEntitiesOptions {
   ...
   * `"subgraph-snapshot"` — override if you've installed the package under a
   * different name.
   */
...
function renderDts(parsed: ParsedSchema, moduleSpecifier: string): string {
  ...
  const banner = [
    "// AUTOGENERATED FROM schema.graphql — do not edit by hand.",
    "// Module augmentation: extends `Entities` in the subgraph-snapshot package",
    "// so `runMatchstickTest` / `Snapshot` are typed without an import.",
  ].join("\n");
...
export async function generateEntities(options: GenerateEntitiesOptions): Promise<void> {
  ...
  const moduleSpecifier = options.moduleSpecifier ?? "subgraph-snapshot";

  const source = await readFile(schemaPath, "utf8");
  const parsed = parseSchema(source);

  await writeIfChanged(outputPath, renderDts(parsed, moduleSpecifier));
}
